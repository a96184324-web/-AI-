...
def handle_image(event):
    with ApiClient(configuration) as api_client:
        ...
        # 【クレジット削減策①】画像サイズを圧縮（リサイズ）してメモリと通信負荷を削減
        # 縦横比を維持したまま、最大1024x1024に縮小
        image.thumbnail((1024, 1024))

        # 【クレジット削減策②】出力（表示）を極力抑えつつ、深い考察を行わせるプロンプト
        prompt = (
            ...
            "・ただし、APIのクレジット（トークン）消費を最小限に抑えるため、出力時は挨拶や無駄な装飾、冗長な説明を完全に省き、要点のみを極めて簡潔に表示してください。\n\n"
            "【条件分岐ルール】\n"
            ...
            "【出力形式】（※クレジット削減のため各項目は短文・箇条書きで出力）\n"
            "1. 競馬場名と展開予想\n"
            "2. 印（◎◯▲☆）と推奨理由（※各馬1文程度。札幌の場合は特別ルール合致点も簡潔に）\n"
            "3. おすすめの買い目\n"
            "※馬券購入は自己責任"
        )
