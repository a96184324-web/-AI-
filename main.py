        ...
        try:
            ...
            def process_race_prediction():
                ...
                prompt = (
                    ...
                    "   ・『順当』：今回と同距離帯（±200m）における持ちタイム・上がり実績が他馬を引き離してダントツであり、前走・前々走でも安定上位の馬が◎の場合に指定せよ。\n"
                    "   ・『混戦』：上位3〜4頭の同距離帯タイム実績が0.1〜0.2秒以内の極めて僅差であり、軸馬の特定が客観的に困難な場合のみに限定指定せよ。\n"
                    "   ・『波乱』：10番人気以下に条件好転・バイアス適合の激走穴馬（☆や△）が存在する場合、または展開崩れ（ハイペース等）で差し・追い込み波乱が見込まれる場合に積極的に指定せよ。\n"
                    "10. 【本線＋メインの2段構え買い目 ＆ 思考文出力完全禁止ルール（絶対上限10点以内）】：\n"
                    "   ・買い目の馬番は、必ず「■ 2. 印・推奨理由」で選定した◎◯▲☆△の馬番のみと100%完全一致・同期させよ！印に存在しない馬番の買い目追加や、◎を買い目から外すことを固く禁止する。\n"
                    "   ・波乱度の判定に応じて、以下の組み合わせで【本線】と【メイン】の2つを出力せよ：\n"
                    "     - 『順当』判定時 ➔ 本線：【馬単 1着固定】（1着◎ ➔ 2着◯,▲：2点） ／ メイン：【3連単 フォーメーション】（1着◎ ➔ 2着◯,▲ ➔ 3着◯,▲,☆,△：4〜6点）［合計6〜8点］\n"
                    "     - 『混戦』判定時 ➔ 本線：【馬連】（◎ ➔ ◯,▲：2点） ／ メイン：【3連複 1頭軸流し】（軸◎ ➔ 相手◯,▲,☆,△から上位4頭：6点）［合計8点］\n"
                    "     - 『波乱』判定時 ➔ 本線：【ワイド】または【馬連】（軸◎または☆ ➔ 相手◯,▲：2点） ／ メイン：【3連複 1頭軸流し】（軸◎ ➔ 相手◯,▲,☆,△から上位4頭：6点）［合計8点］\n"
