...
def handle_image(event):
  with ApiClient(configuration) as api_client:
    ...
    try:
      ...
      # 解像度最適化（認識精度を保ちつつ処理を最速化）
      image.thumbnail((2048, 2048))

      # 究極の予想プロンプト（捏造防止・絶対評価・論理的思考・フォーマット固定）
      prompt = (
          '送られた画像（出馬表など）を解析し、まずは【開催競馬場】と【距離・馬場】を特定してください。\n\n'
          '【絶対厳守事項：ハルシネーション禁止 ＆ 人気・オッズ完全無視】\n'
          '1. 架空のデータや数値をねつ造するハルシネーションは絶対に禁止します。\n'
          '2. オッズや人気順は一切考慮せず、出馬表の事実（枠順、脚質、持ちタイム、騎手、斤量、過去成績）に基づく【条件合致度（100点満点）】を期待度（%）として算出してください。\n'
          ...
          '【全競馬場共通・論理的チェックリスト（捏造防止）】\n'
          '※AIが適当な解釈をしないよう、必ず画像から以下の事実を確認し、論理的な根拠として用いること。\n'
          '・距離と馬場の適性：今回の条件（芝/ダート、距離）と、過去の好走レースの条件が一致しているか。\n'
          '・着差と内容：過去の着順だけでなく「1着とのタイム差」や「上がり3F」を見て実力を評価すること。\n'
          '・展開の競合リスク：前有利なコースでも、過去成績から「逃げ・先行馬」が多すぎると判断した場合は、ハイペースによる共倒れリスクを考慮し、差し馬の評価を相対的に上げること。\n\n'
          '【条件分岐ルール】\n'
          ...
          '（理由を1文程度で。推奨根拠となったタイムや着差、騎手なども具体的に）\n\n'
