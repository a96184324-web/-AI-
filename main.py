...
def handle_image(event):
  with ApiClient(configuration) as api_client:
    ...
    try:
      ...
      # ----------------------------------------------------
      # パターンB: 画像が「出馬表」の場合（ハルシネーション防止強化）
      # ----------------------------------------------------
      else:
        ...
        prompt = (
            ...
            '2. 【馬番と馬名の完全紐付けルール】：表組みを読み取る際、必ず「同じ横の行」にある【馬番】と【馬名】を正しく対応させてください。\n'
            '3. 【騎手名のハルシネーション絶対禁止】：画像から騎手名の文字が100%はっきりと読み取れる場合のみ、馬名の後ろに「（騎手名）」を付与してください。画像が縮小されていて文字が潰れている・読みにくい・不鮮明な場合は、絶対に推測やでっち上げで騎手名を書かず、カッコごと省略して「〇番 馬名」のみで出力してください。\n'
            '4. オッズや人気順は一切考慮せず、出馬表の事実のみに基づく【条件合致度（100点満点）】を期待度（%）として絶対評価で算出してください。\n'
            '5. 【評価基準】S: 90〜100%（条件完全合致）、A: 80〜89%（ほぼ合致）、B: 70〜79%（一部合致）としてください。\n'
            '6. 挨拶や無駄な装飾は省き、箇条書きの「*」記号は絶対に使用しないでください。\n'
            ...
            '・騎手と厩舎：減量記号（▲、★等）の恩恵、継続騎乗、好走騎手への手戻り、厩舎の遠征気配などを加点する。（※画像から読み取れる場合のみ）\n\n'
            '【条件分岐ルール】\n'
