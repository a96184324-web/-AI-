        ...
        try:
            ...
            def process_race_prediction():
                ...
                prompt = (
                    ...
                    "7. 【人気・オッズ非依存】純粋能力＆展開評価ルール（最重要・絶対厳守）：\n"
                    "   ・「1番人気だから」「オッズが低い/高いから」といった人気順やオッズの数字を、本命（◎）や上位評価の判断材料にすることを完全に禁止する（オッズ完全非依存）。\n"
                    "   ・人気に関わらず、「走破タイム・上がり3F限界値」「展開・ペース想定」「本日馬場バイアス」「前走不利・条件変化（どん詰まり・大外ぶん回し・不適性距離からの好転）」のみを客観的に比較・分析せよ。\n"
                    "8. 【◎（本命）と相手（◯〜△）の選定軸明確化ルール】：\n"
                    "   ・『◎本命』(1頭)：単に3着以内に入るだけでなく、1着で勝ち切る能力（決め手・展開適性・脚質）が最も高い馬を必ず1頭選定せよ。\n"
                    "   ・『◯対抗・▲単穴・☆穴馬・△連下』：1〜3着（複勝圏内）に走破する確率（3着内率）が客観的に高い順に配置せよ。\n"
                    "   ・評価表記は『[3着内期待度：〇%]』に完全統一すること（☆穴馬は『[激走期待度：〇%] ※伏兵』）。\n"
                    "   ・△（連下）は出走頭数や混戦度に応じて2〜3頭選定し、全体で6〜7頭の印付き馬を選出してヒモ抜けを防止せよ。\n"
                    "9. レース波乱度の判定と可変評価：\n"
                    "   ・レース概要の波乱度は『順当』『混戦』『波乱』の3段階で判定すること。\n"
                    "     - 『順当』：1着力がダントツな地力・タイム馬を◎に固定。\n"
                    "     - 『混戦』：地力と本日バイアスが合致するハイブリッド評価。\n"
                    "     - 『波乱』：ペース想定（ハイ/スロー）や前走不利からの激走条件が揃った穴馬を積極的に◎・◯・☆に抜擢せよ。\n"
                    "10. 【印自動連動】波乱度に応じた買い目選定ルール（点数厳格上限）：\n"
                    "   ・買い目の馬番は、必ず「■ 2. 印・推奨理由」で選定した◎◯▲☆△の馬番のみと100%完全一致・同期させよ！印に存在しない馬番の買い目追加や、◎を買い目から外すことを固く禁止する。\n"
                    "   ・波乱度に応じて、最も期待値の高い最適な券種スタイル（【馬連流し】【馬単1着固定流し】【3連複フォーメーション】【3連単フォーメーション】【ワイド流し/ボックス】等）を柔軟に自動選定すること。\n"
                    "   ・【買い目点数の厳格上限方針】：\n"
                    "     - 馬連 / 馬単 / ワイド：2〜5点程度\n"
                    "     - 3連複フォーメーション：6〜12点程度（例: 1頭目◎ - 2頭目◯▲☆ - 3頭目◯▲☆△）\n"
                    "     - 3連単フォーメーション：12〜24点程度（例: 1着◎ - 2着◯▲☆ - 3着◯▲☆△）\n"
                    "   ・買い目構成（1着/2着/3着、1頭目/2頭目/3頭目、軸/相手など）は【必ず改行した縦並び】で視認性良く出力すること。\n"
                    ...
                    "（※推奨理由には1着で勝ち切る根拠、地力・タイム面、およびバイアス・展開適合を記述すること）\n"
