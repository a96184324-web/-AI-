        ...
        try:
            ...
            def process_race_prediction():
                ...
                prompt = (
                    ...
                    "6. 【全領域ハルシネーション（誤認識・数値捏造）完全禁止ルール ＆ 画像読み取り不能時のフォールバック】：\n"
                    ...
                    "   ・馬体重やクラス表記等の微小文字が画質・トリミングにより判別不能な場合は、無理に推測・捏造せず、読み取れる走パータイム・上がり・着差・コース条件のみで通常通り判定を完遂せよ。\n"
                    "7. 【照合手順の固定化 ＆ 芝ダ替わり・昇級戦・馬体重の内部評価ルール（人気非依存・大穴救済）】：\n"
                    ...
                    "   ・ステップ3【複合評価（芝ダ替わり・クラス・馬体重・タイム・一変要素）】：\n"
                    "     ① 【芝➔ダート替わり】: 前走「芝」➔ 今回「ダート」への変更馬で、馬体重480kg以上の大型馬、または過去にダート実績がある馬は一変可能性を高く評価し、☆（穴馬）や△（連下）へ積極的に抜擢せよ。\n"
                    "     ② 【昇級戦とクラス緩和】: 前走1着からの昇級初戦馬は、前走タイムが過去データの今回クラス勝ちタイム水準に達しているか確認せよ。未達の場合は評価を抑えよ。逆に前走・前々走が「上級クラス（オープン・重賞等）」で今回自己条件へ条件緩和となる馬は地力上位として優先評価せよ。\n"
                    "     ③ 【馬体重と状態】: 前走比±10kg以上の大幅増減は体調変化を吟味（成長期の馬体重増は許容）。ダート戦における500kg以上の大型馬はパワー・適性加点を行え。\n"
                    "     ④ 人気順やオッズの数字を判断材料にすることを完全に禁止する（1番人気自動固定の徹底排除）。\n"
                    "     ⑤ 近走の見た目の着順（二桁着順など）のみで一律除外することを固く禁止する（10番人気以下の大穴馬を無条件切り捨て禁止）。\n"
                    "8. 【印選定ルール（従来頭数の維持）】：\n"
                    "   ・◎【本命】(1頭)、◯【対抗】(1頭)、▲【単穴】(1頭)、☆【穴馬】(1頭)、△【連下】(2〜3頭) の【計6〜7頭】を選定せよ。\n"
                    "   ・評価表記は『[3着内期待度：〇%]』に完全統一すること（☆穴馬は『[激走期待度：〇%] ※伏兵』）。\n"
                    "9. 【波乱度判定の厳格化ルール（安易な『混戦』逃れの禁止）】：\n"
                    "   ・『迷ったら混戦にする』といったリスク回避的な判定を固く禁止する。\n"
                    "   ・『順当』：◎（本命）が『前走または前々走で1着』かつ『今回と同距離（±200m）で出走馬中トップクラスのタイム実績』を持つ場合に指定せよ。\n"
                    "   ・『波乱』：順当に該当せず、かつ10番人気以下に『前々走同距離1着』や『明確な条件好転（芝ダ替わり・距離変更・馬場回復等）』を持つ激走穴馬（☆や△）が存在する場合、またはハナ争い激化による前崩れ展開が見込まれる場合に積極的に指定せよ。\n"
                    "   ・『混戦』：上記『順当』『波乱』のどちらにも明確に該当しない場合のみに指定せよ。\n"
                    "10. 【本線＋メインの2段構え買い目 ＆ メイン構成自動選択ルール】：\n"
                    "   ・買い目の馬番は、必ず「■ 2. 印・推奨理由」で選定した◎◯▲☆△の馬番のみと100%完全一致・同期させよ！\n"
                    "   ・【本線（2点固定）】：波乱度に応じて以下の2点を出力せよ。\n"
                    "     - 『順当』時 ➔ 馬単 1着固定（1着◎ ➔ 2着◯,▲：2点）\n"
                    "     - 『混戦』『波乱』時 ➔ 馬連 または ワイド（軸◎または☆ ➔ 相手◯,▲：2点）\n"
                    "   ・【メイン買い目の自動選択基準（IF-THEN）】：\n"
                    "     - 【パターンA：フォーメーションを適用（推奨）】➔ 3着候補の連下（△）が2〜3頭存在し、ヒモ荒れを警戒して△を全頭3列目に組み込むべき場合。［1列目:◎ / 2列目:◯,▲ / 3列目:◯,▲,☆,△(全頭)］（計5〜6点）\n"
                    "     - 【パターンB：1頭軸流しを適用】➔ 3着内期待度の高い相手馬が◯,▲,☆,△の4頭に明確に絞り切れ、5番手以下の△馬を押さえる必要がないと判断できる場合。［軸:◎ ➔ 相手:◯,▲,☆,△(4頭のみ)］（計6点）\n"
                    "   ・【思考文・自己修正文の完全出力禁止】：『点数が多くなったので絞る』『ルールに基づき〜修正』などの計算過程、試行錯誤、理由説明の文章を本文中に出力することを完全に禁止する。最終決定した買い目構成と馬番のみをシンプルに出力せよ。\n"
                    "   ・買い目の合計点数はどのような場合でも『最大10点以内』に絶対収めること。\n"
                    "   ・買い目構成（本線・メイン、軸・相手など）は【必ず改行した縦並び】で視認性良く出力すること。\n"
                    ...
                    "△ 【連下】 〇番 馬名（今回騎手名） [3着内期待度：〇%]\n"
                    "△ 【連下】 〇番 馬名（今回騎手名） [3着内期待度：〇%] ※最大3頭目まで可\n\n"
                    "■ 3. おすすめの買い目\n"
                    "【本線】\n"
                    "〇〇（選定券種名）：〇 - 〇, 〇（2点）\n\n"
                    "【メイン】\n"
                    "〇〇（3連複フォーメーション または 3連単/3連複1頭軸流し）\n"
                    "（※フォーメーション時の出力例）\n"
                    "1列目（軸）：〇\n"
                    "2列目：〇, 〇\n"
                    "3列目：〇, 〇, 〇, 〇, 〇\n"
