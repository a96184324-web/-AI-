        ...
        try:
            ...
            def process_race_prediction():
                ...
                prompt = (
                    ...
                    "6. 連対確率最優先＆本質的な穴馬抜擢評価ロジック：\n"
                    "   ・オッズや人気順の数字に流されることを禁止する。\n"
                    "   ・◎【本命】および◯【対抗】には、単勝人気に関わらず「今回のコース適性・持ちタイム水準・展開・走破能力から見て最も連対（1〜2着）確率が高い馬」を純粋・客観的に選定すること（上位人気馬が最も強力であれば素直に本命・対抗にして構わない）。\n"
                    "   ・☆【穴馬】には、前走の展開不利・馬場不向き・今回大幅斤量減・コース一変など「現在のオッズや人気以上に実力・激走ポテンシャルを秘めている伏兵」を明確な数値根拠とともに選定すること。\n"
                    "   ・買い目構成は「3連複」「3連単」「馬単」「馬連」「ワイド」等から堅実度に応じた最適な券種を選び、【必ず改行した縦並び】で視認性良く表示すること。\n"
                    "7. 過去データの照合：スプレッドシートの同条件過去データから勝ちタイム水準・上がり時計・直近の通過順傾向を参照し、今回の出走馬の数値と客観的に比較して根拠に組み込むこと。\n\n"
                    "【新・激走穴馬（☆）抜擢ロジック】\n"
                    "単勝人気や前走着順の数字を過信せず、以下の数値トリガーを満たす伏兵馬を必ず☆（穴馬）または上位印に抜擢すること。\n"
